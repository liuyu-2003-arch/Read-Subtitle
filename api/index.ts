...
// On Vercel, we must use /tmp for writable files, but it's ephemeral.
// For local dev, we use the root directory.
const isVercel = process.env.VERCEL === '1';
const dbPath = isVercel 
  ? path.join("/tmp", "subtitles.db") 
  : path.join(process.cwd(), "subtitles.db");
...
// API: Upload Subtitle
app.post("/api/subtitles/upload", upload.single("file"), (req, res) => {
  try {
    if (!req.file) {
